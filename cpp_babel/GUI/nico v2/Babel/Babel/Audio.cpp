	...
	void *userData)
{
	...
	{*/
		while (++i < framesPerBuffer * 2)
			out[i] = /*p->l_out[0][i];/*/p->out[i];
		/*p->l_out.erase(p->l_out.begin());
	}
	else
	{
		i = -1;
...
{
	int i;
	//float so[FRAMES_PER_BUFFER * 2];

	opus_decode_float(this->dec, sound->str, sound->size, /*so*/this->out, FRAMES_PER_BUFFER, 0);
//	l_out.push_back(so);
	delete sound->str;
	delete sound;
}
